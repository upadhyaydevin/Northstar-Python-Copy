...
#11/13/25
#Currently, this has fixed weighting power of 2, I will now convert it into a parameter
# WEIGHTING_POWER = 2
...
def get_best_fit_angles_deltas(real_detector_responses, real_angles_array,
                                model_detector_responses, model_angles_array, weighting_power):
    """
...
def run_northstar_with_n(event_dict, n_value):

    # Convert scalars to Python floats
    freq = float(event_dict["f"])
    fs   = float(event_dict["fs"])
    T    = float(event_dict["T"])

    true_FH_plus  = float(event_dict["FH_plus"])
    true_FH_cross = float(event_dict["FH_cross"])
    true_FL_plus  = float(event_dict["FL_plus"])
    true_FL_cross = float(event_dict["FL_cross"])
    true_tau      = float(event_dict["tau"])

    # Build model responses
    model_responses, model_angles = generate_model_detector_responses(
        signal_frequency=freq,
        signal_lifetime=0.03,
        detector_sampling_rate=fs,
        gw_max_amps=1.0,
        number_amplitude_combinations=50,
        number_angular_samples=50
    )

    # Build noisy "real" responses
    real_responses, real_angles = generate_real_detector_responses(
        signal_frequency=freq,
        signal_lifetime=0.03,
        detector_sampling_rate=fs,
    ...
    # Weighted matching
    out = get_best_fit_angles_deltas(
        real_responses,
        real_angles,
        model_responses,
        model_angles,
        weighting_power=float(n_value)
    )

    # Derived estimates
    est_F_plus, est_F_cross = beam_pattern_response_functions(hanford_detector_angles, real_angles[0])
    est_tau = time_delay_hanford_to_livingston(real_angles[0])

    # Compute errors
    deltaF_rms = float(np.sqrt(
          (est_F_plus  - true_FH_plus)**2
        + (est_F_cross - true_FH_cross)**2
        + (est_F_plus  - true_FL_plus)**2
        + (est_F_cross - true_FL_cross)**2
    ))

    deltatau_rms = float(abs(est_tau - true_tau))

    return {
        "deltaF_rms": deltaF_rms,
        "deltatau_rms": deltatau_rms
    }
...
if __name__ == "__main__":
    # This script is primarily designed to be called via northstar_hook.py
    # or simulate_stage1.py.
    # The run_northstar_pipeline function is currently not fully functional
    # when run directly due to missing context (e.g., event_dict).
    # To run a full simulation, use simulate_stage1.py.
    # run_northstar_pipeline()
    pass # No direct action when run as main
