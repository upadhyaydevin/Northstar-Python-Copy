...
def generate_real_detector_responses(signal_frequency, signal_lifetime, detector_sampling_rate,
    ...
    weights_h = cp.array([fplus_hanford, fcross_hanford, fplus_hanford, fcross_hanford])
    weights_l = cp.array([fplus_livingston, fcross_livingston, fplus_livingston, fcross_livingston])
    ...
    def show(name, arr):
        print(f"{name:20s} {arr.shape}  {arr.dtype}")
...
def old_source_vector_from_angles(angles) :
...
def old_change_basis_gw_to_ec(source_angles) :
    declination, right_ascension, polarization = source_angles
    src = old_source_vector_from_angles(source_angles)
    z = -1 * src
    y = np.array([
    ...
    contravariant = rot @ vecs
    return np.linalg.inv(contravariant)
#=========================================================================

def _compare(name, truth, batched):
    truth = cp.asarray(truth)
    batched = cp.asarray(batched)
    ok = cp.allclose(truth, batched)
    print(f"\n[{name}] shapes: truth={tuple(truth.shape)} batched={tuple(batched.shape)}")
    print(f"[{name}] allclose: {bool(ok)}")
    if not ok:
        worst = float(cp.abs(truth - batched).max())
        print(f"[{name}] max abs diff: {worst:.3e}  "
              f"({'float noise, loosen tolerance' if worst < 1e-6 else 'REAL mismatch'})")
        print(f"[{name}] truth[0]:\n{truth[0]}")
        print(f"[{name}] batched[0]:\n{batched[0]}")
 

def run_checks(n=5, seed=0):
    # Build test angles on CPU (so the OLD numpy funcs are happy), random and
    # non-trivial so transpose/axis bugs can't hide on zeros or symmetry.
    rng = np.random.default_rng(seed)
    test_cpu = rng.random((n, 3)) * 2.0          # (n, 3) for the OLD loop
    test_gpu = cp.asarray(test_cpu)              # (n, 3) for the NEW batched funcs
 
    # 1) innermost first: source_vector_from_angles
    truth_src = np.stack([old_source_vector_from_angles(test_cpu[k]) for k in range(n)], axis=0)
    batched_src = source_vector_from_angles(test_gpu)        # your batched version
    _compare("source_vector_from_angles", truth_src, batched_src)
 
    # 2) then change_basis_gw_to_ec
    truth_cb = np.stack([old_change_basis_gw_to_ec(test_cpu[k]) for k in range(n)], axis=0)
    batched_cb = change_basis_gw_to_ec(test_gpu)             # your batched version
    _compare("change_basis_gw_to_ec", truth_cb, batched_cb)

if __name__ == "__main__":
    cp.random.seed(0)   # so random draws are reproducible
    result = generate_real_detector_responses(
        signal_frequency=100,
        signal_lifetime=0.03,
        detector_sampling_rate=LIGO_DETECTOR_SAMPLING_RATE,
        gw_max_amps=1,
        number_amplitude_combinations=2,
        number_angular_samples=3,
        max_noise_amp=0.1,
    )
    print("returned", [r.shape for r in result])
